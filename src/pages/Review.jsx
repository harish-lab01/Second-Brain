...
import { Brain, CheckCircle, XCircle, RotateCcw, Sparkles, BookOpen, Zap, Trophy, ChevronRight, Loader2 } from 'lucide-react';
...
export default function Review() {
  ...
  const [phase, setPhase] = useState('loading'); // loading | reviewing | done | empty

  useEffect(() => {
    if (user) {
      Promise.all([fetchNotes(), getUserReviews(user.uid).then(setReviewLogs)]);
    }
  }, [user]);
  ...
  }, [current, dueNotes, phase]);

  const handleRate = async (rating) => {
    if (!dueNotes[current]) return;
    const note = dueNotes[current];

    // Find existing review log for this note to carry forward interval/ef
    const existing = reviewLogs
      .filter(l => l.noteId === note.id)
      .sort((a, b) => new Date(b.reviewedAt?.toDate?.() || b.reviewedAt) - new Date(a.reviewedAt?.toDate?.() || a.reviewedAt))[0];
    ...
    try {
      await saveReviewLog(user.uid, note.id, rating, result.nextReview, result.interval, result.repetitions, result.easeFactor);
    } catch (err) {
      console.warn('Failed to save review log:', err.message);
    }
  ...
  if (phase === 'empty') {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center animate-fade-in">
        ...
        <div className="mt-6 px-5 py-3 rounded-xl bg-white/[0.04] border border-white/[0.07] text-sm text-slate-400">
          <p className="font-medium text-slate-300">{notes.length} notes in your knowledge base</p>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
      ...
      {/* Card */}
      <div
        className="rounded-2xl border border-white/[0.08] bg-surface-50/60 backdrop-blur-sm overflow-hidden"
          ...
          <div className="grid grid-cols-4 gap-3">
            {RATINGS.map(({ value, label, color }) => (
              <button
                ...
                className={`py-3 rounded-xl text-sm font-semibold border transition-all ${color}`}
              >
                {label}
              </button>
            ))}
