...
import { Plus, Search, Sparkles, X, Loader2, Brain, SlidersHorizontal, Zap } from 'lucide-react';
...
export default function Notes() {
  const { user } = useAuth();
  const { notes, loading, fetchNotes, backfillEmbeddings } = useNotes(user?.uid);
  const showToast = useStore(s => s.showToast);

  const [showModal, setShowModal]           = useState(false);
  const [lastSavedNote, setLastSavedNote]   = useState(null); // for connections banner
  ...
  // Filter state for sort/type
  const [filterOpen, setFilterOpen]  = useState(false);
  const [typeFilter, setTypeFilter]  = useState('all'); // all | text | pdf | url
  ...
  // Run semantic search whenever debounced query changes
  useEffect(() => {
    ...
    hybridSearch(debouncedSearch, notes, 20)
      ...
      .finally(() => {
        if (token === abortRef.current) setSemanticLoading(false);
      });
  ...
  const handleBackfill = async () => {
    ...
    if (!missing.length) {
      showToast('All notes already have embeddings!');
      return;
    }
    setBackfilling(true);
  ...
  // Apply tag filter
  if (activeTag !== 'All') {
    displayNotes = displayNotes.filter(n => n.tags?.includes(activeTag));
  }

  // Apply type filter
  if (typeFilter !== 'all') {
    displayNotes = displayNotes.filter(n => n.type === typeFilter);
  }

  // Keyword filter (when NOT in semantic mode or semantic returned nothing)
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex gap-3 items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-11 pr-10 py-3 rounded-xl text-sm text-slate-200 placeholder:text-slate-600
              bg-white/[0.04] border border-white/[0.08] focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/35
              transition-all"
          />
          {search && (
            <button
        ...
        {hasSemanticKey && (
          <button
            ...
            title={semanticMode ? 'Semantic search ON — click to use keyword mode' : 'Click to enable semantic search'}
          ...
          >
            ...
            <span className="hidden sm:inline">{semanticMode ? 'AI' : 'AI'}</span>
          </button>
        )}
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-lg text-xs font-medium border transition-all ${
                    typeFilter === t
                      ...
                      : 'bg-white/[0.03] text-slate-500 border-white/[0.07] hover:text-slate-300'
                  }`}
                >
                  {t.toUpperCase()}
                </button>
      ...
      {/* ── No HF key notice ─────────────────────────────────────────────── */}
      {!hasSemanticKey && (
        <div className="flex items-center gap-2.5 px-4 py-3 rounded-xl bg-yellow-500/5 border border-yellow-500/15 animate-fade-in">
          <Sparkles className="w-4 h-4 text-yellow-400 flex-shrink-0" />
          <p className="text-xs text-slate-400">
            Add <code className="text-yellow-300 bg-yellow-500/10 px-1 rounded">VITE_HF_API_KEY</code> to your <code className="text-yellow-300 bg-yellow-500/10 px-1 rounded">.env</code> to enable AI semantic search.{' '}
            <a
      ...
      {semanticMode && isSearching && (
        <div className="flex items-center gap-2">
          ...
          {semanticLoading && (
            <span className="text-xs text-slate-500">Finding related notes…</span>
          )}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div>
          {/* If semantic search, annotate cards with relevance scores */}
          {semanticResults !== null && semanticMode && displayNotes.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {displayNotes.map(note => (
                <SemanticNoteCard key={note.id} note={note} />
              ))}
            </div>
          ) : (
      ...
      <button
        ...
        className="fixed bottom-8 right-8 w-14 h-14 bg-gradient-to-br from-primary to-primary-700 text-white rounded-full
          shadow-glow hover:shadow-glow transition-all hover:scale-105 active:scale-95
      ...
      {showModal && (
        <AddNoteModal
          ...
          onClose={async () => {
            ...
            await fetchNotes();
            // After fetch, the newest note will be first — show connections banner
            // We delay slightly to let the store update
            setTimeout(() => {
              const currentNotes = useStore.getState().notes;
              if (currentNotes.length > prevCount) {
                setLastSavedNote(currentNotes[0]);
              }
            }, 300);
          }}
        />
...
// Wraps NoteCard with a relevance badge overlay
